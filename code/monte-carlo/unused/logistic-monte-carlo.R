# logistic-monte-carlo.r
# Gabe Ackall, Seongtae Kim, Connor Shrader

# This file contains functions for generating logistic regression data
...
generate_data <- function(n, p, beta = NULL, type = "independent", corr = 0,
  ...
  y <- ifelse(sigmoid(x %*% beta + rnorm(n, sd = st_dev))>0.5, 1, 0)
...
fit_models <- function(dat, n, p) {
  ...
  if (p <= n) {
    # Full model
    fm_time <- system.time(fm <- glm(y ~ ., data = dat, family = "binomial", maxit = 100))
    models[["fm"]] <- fm
    runtimes[["fm"]] <- fm_time
  }
  ...
  if (2 * p <= n) {
    ...
    ridge_time <- system.time(ridge <- cv.glmnet(x = as.matrix(dat[,-1]),
                                                 y = dat$y, alpha = 0, family = "binomial"))
    models[["ridge"]] <- ridge
    runtimes[["ridge"]] <- ridge_time
  }
  
  # Lasso model for variable selection
  lasso_time <- system.time(lasso <- cv.glmnet(x = as.matrix(dat[,-1]), y = dat$y, alpha = 1, family = "binomial"))
  models[["lasso"]] <- lasso
  ...
  enet_time <- system.time(enet <- cv.glmnet(x = as.matrix(dat[,-1]), 
                                             y = dat$y, alpha = 0.8, family = "binomial")) #small alpha is not needed since small multicollinearity
  models[["enet"]] <- enet
  ...
  # MCP
  mcp_time <-  system.time(mcp <- cv.ncvreg(X = dat[, -1], y = dat$y, family = "binomial"))
  models[["mcp"]] <- mcp
...
full_simulation <- function(seed, n, p, beta = NULL, ...) {
  ...
  train_classification_confusion <- lapply(models, confusion_M, dat = train_data)
  train_classification_confusion <- lapply(models, confusion_M, dat = test_data)
  
  # Generate coefficient confusion matrices for linear models.
  confusion_matrices <- confusion_matrices(coefficients_table)
  
  return(list(
    coefficients = coefficients_table, 
    #models = models,
    train_class_confusion = train_classification_confusion,
    test_class_confusion = test_classification_confusion,
    coefs_confusion = confusion_matrices, runtimes = runtimes))
}
...
monte_carlo <- function(n, p, iterations,
  ...
  tryCatch({
    ...
    clusterExport(cl, list("n", "p", "iterations", "...", "generate_coefficients",
                           ...
                           "confusion_M", "individual_confusion_matrix",
                           "confusion_matrices"),
                  envir = environment())
