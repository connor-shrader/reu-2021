...
\begin{document}
	...
	\textit{Ridge regression} is a penalized regression model that uses the penalty function $P(\beta) = \beta^2$ \cite{hoerl1970ridge} in Equation \ref{eqn:penalized-regression-lambda}. 
	\begin{comment}
	...
	The \textit{least absolute shrinkage and selection operation}, commonly referred to as \textit{lasso}, is a shrinkage method with a very similar form to ridge regression \cite{tibshirani1996regression, james2013introduction}. The penalty function for lasso is $P(\beta) = \vert \beta \vert$. Unlike ridge regression, the lasso can perform variable selection.
	...
	\textit{Elastic-net} (or E-net) regression uses both the ridge penalty and the lasso penalty at the same time by adding their two penalties (possibly with different tuning parameters $\lambda_1$ and $\lambda_2$) \cite{zou2005regularization}.
	...
	\begin{table}[h!]
		...
		\begin{tabular}{l|ll|ll}
			...
			MCP  & $0.1566$ & $0.0129$ & $\mathbf{0.2202}$ & $\mathbf{0.0224}$ \\
			GBM  & $\mathbf{0.0002}$ & $\mathbf{0.0004}$ & $0.2233$ & $0.0507$ \\
	...
	We first discuss in detail our findings from the simulations where $n = 50$ and $p = 2000$. Then, we discuss some of the broader results that arose for other values of $n$ and $p$. We defer the reader to our supplementary document to see the figures and tables for the remaining simulations.
	...
	\begin{itemize}
		\item When the response is linear, penalized linear models have a lower MSE on test data than non-linear models.
		\item Non-linear models had a lower MSE on training data than linear models in almost all the simulations.
		\item When the response is non-linear, penalized linear models generally have a slightly higher test MSE when $p > n$ when compared to non-linear models. When $n > p$, the linear models had a significantly higher test MSE.
		...
		\item The linear models struggled to identify important predictors when the response was non-linear.
