...
One way to modify the lasso method is to use the \textit{smoothly clipped absolute deviation} (SCAD) penalty.
\begin{equation}
...
\end{equation}
where $\lambda \geq 0$ and $a\geq 2$ are tuning parameters. This is equivalent to
\begin{equation}
	\frac{dJ_a(\beta, \lambda)}{d\beta}\left\{\begin{array}{ll}
		\lambda,&\vert \beta \vert\leq \lambda\\
		\frac{a\lambda - \vert \beta \vert}{a - 1},&\lambda < \vert \beta \vert < \alpha\lambda\\
