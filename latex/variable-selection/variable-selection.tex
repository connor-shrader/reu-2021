...
\maketitle
\begin{abstract}
	With the prevalence of big data in the modern age, the importance of modeling high dimensionality data has increased greatly. This data has traditionally been modeled through the use of random forests, gradient boosting models, or support vector machines. In this paper, we will investigate the use of penalized regression methods instead of, or in conjunction with, the traditional machine learning methods. We will focus on lasso, ridge, and elastic net for our penalized regression models.\\
	
	\textit{Keywords}: penalized regression, variable selection
\end{abstract}
...
Finally, unstructured covariance assumes that $\mathbf{\Sigma}$ takes no structured form. We have
\begin{equation}
	\mathbf{\Sigma} = \begin{bmatrix}
...
\subsubsection{Subset Selection Models}
We fitted subset selection models (such as stepwise forward and stepwise backward selection) using the \lstinline!stepAIC! function from the \lstinline!MASS! package. For each subset selection algorithm, we fitted two different models: one using the Akaike Information Criterion (AIC) and one using the Bayesian Information Criterion (BIC). This gave us a total of eight subset selection models. We also evaluated the full OLS model (containing all predictors) and the null model (containing no predictors).

In the case where $p>n$, the full model cannot be computated. Furthermore, due to the functionality of \lstinline!stepAIC!, we did not compute the subset selection models in the case where $2p>n$. Hence, when $2p>n$, the only models computed were the null model, the penalized regression models, and the non-linear models.

\subsubsection{Penalized Regression Models}
The lasso, ridge, and elastic-net models were computed using the \lstinline!glmnet! library. We used the \lstinline!cv.glmnet! function, which automatically selects the optimal value of $\lambda$ that minimizes the test mean-squared error (using cross-validation). However, \lstinline!glmnet! only tests $\lambda$ in a certain range, so it is possible that the function misses the best possible value of $\lambda$. To avoid this issue, we manually checked the values of $\lambda$ chosen by \lstinline!glmnet! to make sure that none of them were corner cases.
...
For SCAD and MCP, we used the \lstinline!ncvreg! library. Like \lstinline!glmnet!, \lstinline!ncvreg! contains a cross-validation function \lstinline!cv.ncvreg! to find the value of $\lambda$ that minimizes the mean-squared error. By default, \lstinline!ncvreg! uses $\gamma = 3.7$ for SCAD and $\gamma = 3$ for MCP.

\subsubsection{Non-Linear Models}
To fit models using random forests and gradient boosting, we used grid search. This means that we generated models using many different combinations of hyperparameters and selected the combination that made the best predictions for a validation set. The resulting model was then used to make predictions on the test set to evaluate performance. Tables \ref{tab:rf-parameters} and \ref{tab:gb-parameters} below show the choices of hyperparameters we turned with grid search.

\begin{table}[h]
	\caption{Choice of hyperparameter values for random forest grid search.}
...
\begin{table}[h]
	\caption{Choice of hyperparameter values for gradient boosting grid search.}
