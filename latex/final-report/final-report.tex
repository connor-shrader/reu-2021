...
\end{equation}
where the first summation is the usual residual sum of squares, $\lambda$ is a hyperparameter that controls the strength of the penalty, and $P(\beta_j)$ is a penalty function applied to each of the coefficients (but not the intercept term). In general, $P(\beta_j)$ is an even function that is non-decreasing as $\vert \beta_j \vert$ increases. If $\lambda = 0$, then we have the usual ordinary least squares estimator. As $\lambda$ increases, a stronger penalty is applied which will decrease the coefficient values. As $\lambda$ approaches $\infty$, the model becomes an empty model where only the intercept term is non-zero.
...
\end{equation}
where $t$ is a hyperparameter. In this form, penalized regression fits the ordinary least squares such that the sum of the penalties of the coefficients are kept below some threshold. It can be shown that for eery value of $\lambda$ from Equation \ref{eqn:penalized-regression-lambda}, there is a value of $t$ from \ref{eqn:penalized-regression-t} that gives equivalent coefficient estimates \cite{james2017islr}. Most algorithms to fit penalized regression models use the form given in Equation \ref{eqn:penalized-regression-lambda}, but the form in Equation \ref{eqn:penalized-regression-t} may be considered more intuitive.
...
\textbf{Ridge regression} is a penalized regression model that uses the penalty function $P(\beta_j) = \beta_j^2$ \cite{hoerl1970ridge}. One major benefit of ridge regression is that it has a closed form solution. Letting $\beta$ be a $(p + 1)\times 1$ vector of coefficients, $\mathbf{X}$ a $n\times (p + 1)$ matrix of data and $\mathbf{y}$ a $n\times 1$ vector of response values, the coefficient estimates using ridge regression are
\begin{equation}\label{eqn:ridge-solution}
	TODO
\end{equation}
Ridge regression has some additional benefits over ordinary least squares. One advantage is that ridge regression can handle highly correlated data better than ordinary least squares. When predictors are correlated with each other, some algorithms have a hard time distinguishing which predictors are actually related to the response. One other benefit of ridge regression is that it can be used when $p>n$.
...
The \textbf{least absolute shrinkage and selection operation}, often referred to as \textbf{lasso}, is a shrinkage method with a very similar form to lasso regression \cite{tibshirani1996regression, james2017islr, james2013introduction}. The penalty function for lasso is $P(\beta_j) = \vert \beta_j \vert$.
