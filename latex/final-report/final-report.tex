...
\begin{equation}\label{eqn:penalized-regression-lambda}
	\hat{\beta}=\argmin{\beta}{\sum\limits_{i = 1}^N \Big[y_i - (\beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \cdots + \beta_p x_{ip})\Big]^2 + \lambda\sum\limits_{j = 1}^p P(\beta_j)}
\end{equation}
...
\begin{equation}\label{eqn:penalized-regression-t}
	\hat{\beta} = \argmin{\beta}{\sum\limits_{i = 1}^N \Big[y_i - (\beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \cdots + \beta_p x_{ip})\Big]^2} \quad \text{subject to} \quad \sum\limits_{j = 1}^p P(\beta_j)\leq t
\end{equation}
...
Subset selection models using forward, backward, stepwise forward, and stepwise backward selections were fitted using the \lstinline!MASS! library. For each of these four algorithms, we fit models using both AIC and BIC, giving up to eight models total.
...
Some of the models used could only be used for certain values of $n$ and $p$. This is because either the runtime becomes unreasonable when $n$ or $p$ are large, or the model simply cannot be used when $p$ is too large. Ordinary least squares was used when $p\leq n$, since it cannot be used at all when $p>n$. For the same reason, the forward subset selection algorithms were also used when $p\leq n$. The backward subset selection algorithms were only used when $p\leq n$ and $p\leq 40$. When $p>40$, the runtimes for backward selection and backward stepwise selection were very long, so we chose to discard backward selection when $p$ was too large. Ridge regression was used when $p\leq n$ as well as it is not designed for variable selection or for $p \geq n$ scenarios.
...
\begin{landscape}
	...
	\captionof{table}{Mean and standard deviation of the mean squared error on training data when $n = 1000$ and $p = 10$. See Figure \ref{fig:train-mse-50-2000} for the corresponding plot.}\label{tab:train-mse-1000-10}
	\input{./tables/train_mse_1000_10.tex}
	
	\newpage
	
	\captionof{table}{Mean and standard deviation of the mean squared error on test data when $n = 1000$ and $p = 10$. See Figure \ref{fig:test-mse-50-2000} for the corresponding plot.}\label{tab:test-mse-1000-10}
	\input{./tables/test_mse_1000_10.tex}
	
	\newpage
	\captionof{table}{Mean and standard deviation of the $\beta$-sensitivity when $n = 1000$ and $p = 10$. See Figure \ref{fig:sensitivity-1000-10} for the corresponding plot.}\label{tab:sensitivity-1000-10}
	\input{./tables/sensitivity_1000_10.tex}
	
	\newpage
	
	\captionof{table}{Mean and standard deviation of the $\beta$-specificity when $n = 1000$ and $p = 10$. See Figure \ref{fig:specificity-1000-10} for the corresponding plot.}\label{tab:specificity-1000-10}
	\input{./tables/specificity_1000_10.tex}
	
	\newpage
	
	\captionof{table}{Mean and standard deviation of the mean squared error on training data when $n = 50$ and $p = 2000$. See Figure \ref{fig:train-mse-50-2000} for the corresponding plot.}\label{tab:train-mse-50-2000}
	\input{./tables/train_mse_50_2000.tex}
	
	\newpage
	
	\captionof{table}{Mean and standard deviation of the mean squared error on training data when $n = 50$ and $p = 2000$. See Figure \ref{fig:test-mse-50-2000} for the corresponding plot.}\label{tab:test-mse-50-2000}
	\input{./tables/test_mse_50_2000.tex}
	
	\newpage
	
	\captionof{table}{Mean and standard deviation of the $\beta$-sensitivity when $n = 50$ and $p = 2000$. See Figure \ref{fig:sensitivity-50-2000} for the corresponding plot.}\label{tab:sensitivity-50-2000}
	\input{./tables/sensitivity_50_2000.tex}
	
	\bigskip
	
	\captionof{table}{Mean and standard deviation of the $\beta$-specificity when $n = 50$ and $p = 2000$. See Figure \ref{fig:specificity-50-2000} for the corresponding plot.}
