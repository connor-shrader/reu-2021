...
Looking at Figures \ref{fig:train-mse-1000-10} and \ref{fig:test-mse-1000-10}, we see that the linear models have very similar train and test mean-squared errors. For these models, the mean squared error is approximately equal to $\sigma^2$, the square of the random error. We also see that for all of the linear models except for ridge regression, the type and strength of correlation has little effect on model performance. However, ridge regression performs worse when the correlation is high.
...
We see in Figures \ref{fig:train-mse-50-2000} and \ref{fig:test-mse-50-2000} that the mean squared error for lasso and elastic-net are generally larger than SCAD and MCP for both the training data and test data. Gradient boosting has almost zero training mean squared error under all conditions, but has a relatively large test error. Random forest and support vector machine models have a moderate training error but a large test error. Interestingly, we see that the non-linear models all perform better when there is a strong correlation between predictors. On the other hand, the linear models are somewhat less affected by the correlation.

Recall that in the design of the Monte Carlo simulations, five of the predictors are truly correlated with the response: $\beta_0$ (the intercept), $\beta_1$, $\beta_2$, $\beta_5$ and $\beta_6$. Figures \ref{fig:fn-50-2000} measures the average number of predictors that were predicted by the penalized regression models to be zero that were actually non-zero. In other words, this figure measures the inability for each model to recognize important predictors.

\begin{figure}[h!]
	\centering
	\includegraphics[width = \textwidth]{images/facet-fn/facet_fn_50_2000.eps}
	\caption{Average Mean Square Error for Empirical Data Models}
	\label{fig:fn-50-2000}
\end{figure}

We see that when there is little correlation between predictors, SCAD and MCP predict the fewest false negatives, meaning that they were more likely to recognize predictors that are related to the response. We also see that in general, stronger correlation inhibits the ability for all of the models to make correct predictions. In fact, when there is a high correlation, SCAD and MCP have the most false negatives.
...
\begin{tabular}{lllll}
	\hline
	& \multicolumn{2}{l}{Train} & \multicolumn{2}{l}{Test} \\ 
	Model  & Average MSE & SD & Average MSE & \multicolumn{1}{l}{SD} \\ 
	\hline
	Ridge  & $0.1391293$ & $0.0265920$ & $0.2858$ & $0.06098$ \\
	Lasso  & $0.1841874$ & $0.0159230$ & $0.2304$ & $0.03371$ \\
	Elas. Net  & $0.1798794$ & $0.0127395$ & $0.2281$ & $0.04690$ \\
	SCAD  & $0.1441726$ & $0.0332773$ & $0.2218$ & $0.03027$ \\
	MCP  & $0.1566425$ & $0.0129121$ & $0.2202$ & $0.02243$ \\
	XGBoost  & $0.0001611$ & $0.0003601$ & $0.2233$ & $0.05073$ \\
	Random Forest  & $0.0377885$ & $0.0013448$ & $0.2653$ & $0.05253$ \\
	\hline 
\end{tabular}
\end{table}
