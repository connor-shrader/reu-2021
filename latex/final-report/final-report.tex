...
Note that of the penalized regression methods discussed above, only ridge regression has a general closed-form solution. Although LASSO, elastic-net, SCAD and MCP do not have closed-form solutions, their solutions can be efficiently approximated. In some special cases, however, a closed-form solution exists. In an \textbf{orthonormal design}, we assume that the $n\times p$ matrix $\mathbf{X}$ containing $n$ observations of $p$ predictors is orthonormal. This means that the magnitude of each column of $\mathbf{X}$ is 1 and every pair of columns of $\mathbf{X}$ is orthogonal. To simplify things, we can ignore the extra column of 1's that would correspond to the intercept term. In this special case, the value for each coefficient is independent of the other coefficients; in other words, we can compute each coefficient value as if it were the only predictor in the data set.

In this special case, we can apply a \textbf{thresholding function} to each predictor to get the coefficient estimate for that predictor. The input to the thresholding function is the actual coefficient value, and the output is the estimated value for that coefficient in an orthonormal design. Figure \ref{fig:prediction} shows the threshold functions for LASSO, SCAD and MCP when $\lambda = 2$. For SCAD and MCP, we also used the value $a = 3$. For reference, we included the identity line, which corresponds to the threshold function for ordinary least squares.
