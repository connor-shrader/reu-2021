...
If we wish to produce a model that makes reliable predictions on test data, we must use a different performance metric. Two of the most common metrics used for this purpose are the \textbf{Akaike Information Criterion} (AIC) and the \textbf{Bayesian Information Criterion} (BIC). Both of these metrics work by adding the residual sum of squares with a penalty term that punishes models with many predictors. As a result, the model selected using AIC or BIC will typically contain fewer predictors.
